document.addEventListener('DOMContentLoaded', () => {
    ...
    const backgroundImageUrl = 'vids/g.png';  // Set your background image URL here
    const videoOpacity = 1; // Set your desired video opacity (0.0 to 1.0)

    // New auto-refresh boolean variable
    ...
    function randomizeVideoShapes() {
        const shapes = ['circle', 'rectangle', 'triangle'];
        videoElements.forEach(video => {
            const randomShape = shapes[Math.floor(Math.random() * shapes.length)];
            video.className = `video-player ${randomShape}`;
        });
    }
    ...
    function randomizeVideos() {
        ...
        randomizeVideoShapes(); // Apply random shapes
    }
